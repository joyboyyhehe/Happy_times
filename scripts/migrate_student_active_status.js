...
async function migrate() {
  ...
  for (const s of students) {
    if (s.active === undefined || s.archived === undefined) {
      console.log(`Updating student "${s.name}" (ID: ${s.id}): setting active: true, archived: false`);
      await updateDoc(doc(db, 'students', s.id), {
        active: s.active !== undefined ? s.active : true,
        archived: s.archived !== undefined ? s.archived : false
      });
      updatedCount++;
    }
  }
