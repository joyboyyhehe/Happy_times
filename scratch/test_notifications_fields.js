...
async function test() {
  ...
  // Set FCM token with extra fields
  console.log(`Setting FCM token with extra fields for parent: ${parentUid}`);
  await setDoc(doc(db, 'fcm_tokens', parentUid), {
    token: 'dummy_token_12345_fields_test',
    updatedAt: new Date(),
    userId: parentUid,
    uid: parentUid,
    parentUid: parentUid,
  ...
  const postRef = await addDoc(collection(db, 'posts'), {
    title: 'Test Fields Notification',
    body: 'This is a test to check if Cloud Functions pick up the token with fields.',
    category: 'General',
    scope: 'class',
